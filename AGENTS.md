...
- **Timestamps:** To ensure the timeline of thoughts survives a rebuild, `daemon.py` explicitly injects YAML frontmatter (`created_at`/`updated_at`) into generated markdown files. `rebuild.py` parses original unix timestamps off archive filenames and passes them to `process_file_core` to preserve exact ingestion times.
...
- **Continuous Ledger:** `daemon.py` appends every successful action to `log.md` (which is automatically rotated monthly into `archive/logs/log_YYYY_MM.md`). `lint_wiki.py` reads the last 7 days of this log to generate its "Weekly Timeline" narrative.
